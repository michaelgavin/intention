...
The fun thing about this first case study is that it's basically a stroll through the uncanny valley of digital literary studies. There a very strong assumption in quantitative analysis that what's being studied is a *real thing out there in the real world* and that the model is an *imperfect representation of that thing*. This is a misundertanding of modeling's basic metaphysical problem. We might choose to imagine that a model is a representation of some real thing, but that's an ontological commitment we bring to bear on the object, not a property of the object itself.

This metaphysical problem in the epistemology of modeling is very similar to the problem that plagues twentieth-century debates on the topic of authorial intention. Just as it's perfectly possible and often plausible to behave as if statistics represent something real outside themselves, it's perfectly possible and often plausible to behave as if discourse represents a reality outside itself (an intention, perhaps). In either case, though, there's nothing about discourse that compels such a commitment.

This is why I describe reality as an imaginary apparatus that surrounds explanation. Realities are produced by explanations, and intentions are produced by the critical discourse that names them. (Here I'm riffing of G.E.M. Anscombe more than anyone else.) This is true whether one maintains an intentionalist view of language or an anti-intentionalist view. Either way, you're stuck producing the intentions about which you hope to argue -- this was true of Searle and Derrida and their pointless disagreements, and it'll be true for anyone. It doesn't matter whether you believe what I'm saying here, because you are bound up in the same inexorable logic, versions of which have plagued the history of literary criticism since its inception. (Just ask Jonathan Swift, who coped with this dilemma by attributing it to bad faith.)

For someone doing quantitative analysis, this is actually kind of freeing, because it means that your models are free to produce logics internal to themselves, and you're perfectly free all the same to believe that those logics maintain some analogous relation to the realities you imagine for them.

In practice, if you're doing corpus analytics, this means that the "findings" of your empirical tests will always depend on choices made during data curation and analysis. Everybody knows this and everybody says it, but it's really hard to keep in mind when concrete results of statistical tests seem so close to something like the truth. This why I call it the "uncanny valley of valid explanation" Just as the intentions a biographically inclined scholar finds in a text are produced by the work of contextualization, so too the intentions found by any mathematical procedure will be produced through the act of analysis.

What was fun about writing the Locke case study was the pleasure of scurrying around in the nooks and crannies of the space of this intellectual discomfort. On one hand, I unapologetically refer to my findings as representations of Locke's intent. (They're more trustworthy then any exemplary quotations *you* might choose.) On the other hand, it's inescapably obvious that the findings are produced by the methodology, and it's bizarrely at odds with common sense that changing the corpus of comparison would change Locke's intent.

digression concluded ...

### Table 1.1
The basic idea of this case study is to perform two related sets of comparisons. I want to compare Locke's *Treatise* to the EEBO model, as above, but I also want to parse into it in a little more detail. Based on what data scientists call my "domain expertise," I knew that Locke joined a vibrant field of debate around questions of natural law, and based on my (whaddaya call it?) "technical good guessery," I figured that it'd be trivially simple to recover documents in that field of debate by just finding those that use words in roughly the same proportion that Locke uses them. Once that subcorpus is identified, I triangulate the conceptual work scores for Locke against those of his peers.
...
```
Note that Table 1.1 ranks documents by *distance* but the function above returns *similarity*, so to reproduce the numbers in the table itself you'll have to run the command `locke_ids = 1 - locke_ids`.
...
natlaw_freqs = rowSums(deebo[,names(locke_ids)])
...
```r
similarity_map(m, "children", numResults = 21, numGrps = 4)
similarity_map(eebo, "children", numResults = 21, numGrps = 4)
```
