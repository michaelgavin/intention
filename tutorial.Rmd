...
This metaphysical problem in the epistemology of modeling is very similar to the problem that plagues twentieth-century debates on the topic of authorial intention. The weird thing about a quantitative theory of intention is that obtrudes onto our awareness the process by which scholarship creates the objects to which we attach symbolic veridicality. By calling it a theory of "intention," I'm forcing the issue. 
...
Here are the scripts used to build the model.

```r
```

At the end are two matrices. The dataset `deebo` is replaced by another called `yearly`, which gathers the frequencies and another, named `teebo`, which ...

To make Figure 8, you just :
