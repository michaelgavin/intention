...
Assuming that you have R installed and the `devtools` package loaded (as well as the `ggplot2` and `Matrix` packages), you can get everything you need by first cloning the repository and then running these commands.

To clone the directory, go to https://github.com/michaelgavin/intention and click "Clone or Download."
NOTE: Because of how the data is structured, the `install_github()` function doesn't work. You need to clone the repository and install from your local directory.

```r
# You'll have to change the filepath to your local directory
devtools::install("~/Documents/Github/intention")
```
...
```r
load(system.file("extdata", "A48901.rda", package = "intention"))
...
```r
load(system.file("extdata", "A26981.rda", package = "intention"))
similarity(m, "children")
```
...
```r
# Don't run these commands unless you've downloaded the complete dataset from Dataverse
...
```r
curve_of_semantic_normality = threshold$delta + (threshold$mu * threshold$phi)
```
...
# Load Paradise Lost
load(system.file("extdata", "A50919.rda", package = "intention"))
...
curve_of_semantic_normality = (threshold$mu * threshold$phi) + threshold$delta
...
```r
load(system.file("extdata", "natlaw.rda", package = "intention"))
```
...
sort(triangulated_work, decreasing = T)[1:5]
```
...
```r
load(system.file("extdata", "behn_mat.rda", package = "intention"))
...
```r
load(system.file("extdata", "A27301.rda", package = "intention"))
...
```r
load(system.file("extdata", "teebo.rda", package = "intention"))
...
It's been very exciting the last few years to see how the field of cultural analytics has developed. It wasn't that long ago -- 2014 or 2015 -- that sniping critics writing in bad faith could thump their chests on social media and say computational literary studies is a worthless field that hadn't accomplished anything. And, for sure, I wouldn't want to go back to the 1990s and early 2000s, when the standard of critical sophistication involved opining about hypertext. But sometimes I feel like something has been lost, that we're all getting a little too serious, too modest, too content with proving, yet again, that yes, you really can learn something with digital methods.

I don't want to "really learn something." I want to rethink everything. I want to be transformed. I want to be impinged upon. I want to be crafted. I want to feel my mind unfurl across the contours of alien ideas.
